...
class ProductionPushCert
  ...
  class << self
    ...
    def pem_path_for_app_key(app_key)
      File.join(OKConfig[:apns_pem_path], "#{app_key}.pem")
    end
  end
  ...
  def save
    ...
    if p12 && p12.tempfile
      ...
      if !passing
        ...
        errors.add(:base, "Could not read your production certificate.  Please enter the password that you used to create the .p12 file.")
      end
    end
    passing
  end
